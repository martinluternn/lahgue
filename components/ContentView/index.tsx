...
const ContentViewComponent: React.FC<{ feed: number }> = ({ feed }) => {
  const [data, setData] = useState<MainContentEntity[]>([]);
  ...
  const fetchData = async () => {
    ...
    const result = await axios.get(`https://lahelu.com/api/post/get-posts`, {
      params: {
        feed: feed,
        page: page.toString(),
      },
    ...
    if (json) {
      setPage(page + 1);
      const newData: MainContentEntity[] = json.postInfos?.map((el: any) => ({
        key: el.title + el.feed,
        commentCount: el.totalComments,
        image: `https://cache.lahelu.com/${el.media}`,
        lastTime: el.userPlusTime,
        profileName: el.userUsername,
        tags: el.hashtags,
        title: el.title,
        upVoteCount: el.totalUpvotes,
      }));
      setHasMore(json.hasMore as boolean);
  ...
  const renderItem = ({ item }: { item: MainContentEntity }) => {
    return (
      <View key={item.key}>
        <TouchableOpacity>
          <View style={contentViewStyle.container}>
            <View style={contentViewStyle.subContainer}>
              ...
              <Text style={contentViewStyle.date}>{" · " + formatTimeDifference(parseInt(item.lastTime, 0))}</Text>
            </View>
            <TouchableOpacity>
              <Svg
                width={16}
          ...
          <View style={contentViewStyle.sectionOnePost}>
            ...
            {item.tags.map((tag) => (
              ...
              >
                ...
                <Text style={contentViewStyle.titleSawer}>{tag}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <View style={contentViewStyle.sectionTwoContainer}>
            <View style={contentViewStyle.rowContainer}>
              <TouchableOpacity style={contentViewStyle.upVote}>
                <Svg
                  width={16}
                  height={16}
                  fill={ThemeColor.white}
              ...
              <TouchableOpacity style={contentViewStyle.downVote}>
                <Svg
                  width={16}
                  height={16}
                  fill={ThemeColor.white}
  ...
  const ListEndLoader = () => {
    if (!isFirstPageReceived && isLoadingData) {
